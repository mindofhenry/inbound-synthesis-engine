...
// Build per-contact + anonymous bucket of raw signals.
const now = Date.now();
...
// Rank with the same comparator as WF1 post-MIN-77:
// (a.tier - b.tier) || (b.fp_sum - a.fp_sum) || (a.recency - b.recency)
...
// Signal phrase: dedupe by event type, keep first (highest points after sort), comma-join.
function phrase(sig) {
...
// Build Block Kit blocks: header section + one section per non-primary contact
// + optional anonymous-third-party section + footer context.
const blocks = [];
blocks.push({
  type: 'section',
...
const stickyWebhook = node({
  ...
  config: {
    name: 'Sticky Webhook',
    parameters: {
      content: '## Slack interactivity ingress\n\nReceives the "Show contact details" button click. POST to /webhook/contact-details with form-encoded payload=<json>. responseMode default (immediate 200) keeps under Slack\'s 3s budget; the actual reply is posted async via response_url.',
      height: 220,
      width: 360,
      color: 6
    },
...
export default workflow('contact-details-callback', 'Contact Details Callback')
